...
Write-ColorOutput Yellow "`n🚀 Starting deployment of Cloud Functions to GCP"
...
if ($failedFunctions.Count -gt 0) {
    Write-ColorOutput Red "❌ Failed functions:"
    foreach ($failed in $failedFunctions) {
        Write-ColorOutput Red "   - $failed"
    }
    
    Write-ColorOutput Yellow "`n🔧 To retry failed functions individually, run:"
    foreach ($failed in $failedFunctions) {
...
Write-ColorOutput Yellow "`n📝 Useful commands:"
...
Write-ColorOutput Green "`n✨ Deployment script completed!"
